...
# savepath    = sys.argv[1]	# Start from 1, 0 is about the strucutre of the passes arguments or sth
...
for ii in range(Ntrials):
    ...
    mon_states_e = StateMonitor(Pe, ['v', 'x', 's_nmda'], record=[1200], dt=dt_state_mon)
    # mon_states_e_nmda = StateMonitor(Pe, ['s_tot', 's_nmda', 's_ext'], record=True, clock=state_mon_clock)
    # print mon_states_e_nmda.s_ext

    # Run Simulations
    net = Network(Pe,Pe0,Pe1,Pe2,Pi,PG0,PG1,PG2,PGi,selfnmda,Cp0,Cp1,Cp2,Cpi,Cie,Cii,Cei,C00,C10,C20,C01,C11,C21,C02,C12,C22,update_nmda,monit_P1,mon_states_e)
    ...
    np.save('state_e_x_'+suffix+'_cue_'+str(coherence)+'_run_'+str(ii)+'.npy', np.column_stack((mon_states_e.t, mon_states_e_v_mean, mon_states_e_x_mean)))
